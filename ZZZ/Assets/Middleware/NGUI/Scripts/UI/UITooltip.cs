...
public class UITooltip : MonoBehaviour
{
	...
	{
		SetText(tooltipText, Input.mousePosition, new Bounds(Vector3.zero, Vector3.zero));
	}

	protected virtual void SetTextStatic(string tooltipText)
	{
		SetText(tooltipText, mTrans.localPosition, new Bounds(Vector3.zero, Vector3.zero));
	}

	protected virtual void SetText(string tooltipText, Vector3 position, Bounds bounds)
	{
		if (mCurrent > 0.5f)
		{
			mCurrent = 0.5f;
		...
		if (text != null && !string.IsNullOrEmpty(tooltipText))
		{
			...
			if (background != null)
			{
				...
				mSize.x += border.x + border.z;// +Mathf.Abs(offset.x - border.x) * 2f;
				mSize.y += border.y + border.w;// +Mathf.Abs(-offset.y - border.y) * 2f;
			...
			if (uiCamera != null)
			{
				...
				mPos.x = Mathf.Clamp01((mPos.x - 5) / Screen.width);
				mPos.y = Mathf.Clamp01(mPos.y / Screen.height);

				float rightX = (bounds.center.x + bounds.size.x / 2) / Screen.width;
				float totalSizeX = bounds.size.x / Screen.width;

				//Debug.Log(bounds.size + " " + bounds.center);
				...
				mPos.x += rightX;
				if (mPos.x > 1.0f - max.x) mPos.x -= totalSizeX + max.x;
				if (mPos.y > 1.0f - max.y) mPos.y = 1.0f - max.y;
	...
	static public void ShowTextStatic(string tooltipText)
	{
		if (mInstance != null)
		{
			mInstance.SetTextStatic(tooltipText);
		}
	}

	static public void ShowText(string tooltipText, Vector3 position)
	{
		if (mInstance != null)
		{
			mInstance.SetText(tooltipText, position, new Bounds(Vector3.zero, new Vector3(50, 50)));
		}
	}

	static public void ShowText(string tooltipText, Vector3 position, Bounds bounds)
	{
		if (mInstance != null)
		{
			mInstance.SetText(tooltipText, position, bounds);
		}
	}

	public bool forceHide()
	{
		return false;// HexUI.isVisible() || EventUI.isVisible();
	}
}
